...
static INT_PARAM_FLAG(save_freq, 0, "If set, store the model every this many iterations");
...
int main(int argc, char **argv) {
  ...
  if(FLAGS_save_freq > kNumPagesPerBatch)
  {
  	tprintf("save frequency larger: \n");
  }
  else
  {    
        tprintf("save frequency smaller: \n");
  }
  
  tesseract::LSTMTrainer trainer(
      FLAGS_model_output.c_str(),
      checkpoint_file.c_str(), FLAGS_debug_interval, FLAGS_save_freq,
  ...
  do {
    ...
    for (int target_iteration = iteration + FLAGS_save_freq;
