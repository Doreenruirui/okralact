...
LSTMTrainer::LSTMTrainer(const char* model_base, const char* checkpoint_name,
                         int debug_interval, int save_freq, int64_t max_memory)
    : randomly_rotate_(false),
...
bool LSTMTrainer::MaintainCheckpoints(TestCallback tester, STRING* log_msg) {
  ...
  if (checkpoint_name_.length() > 0) {
    ...
    STRING iter_name_ = model_base_.string();
    iter_name_.add_str_int("_", iteration);
    iter_name_ += ".checkpoint"; 
    if (!SaveTrainingDump(FULL, this, &checkpoint) ||
        !SaveDataToFile(checkpoint, iter_name_)) {
    ...
    *log_msg += "\n";
    ...
    while (sub_trainer_->training_iteration() < end_iteration &&
      ...
      int target_iteration =
          sub_trainer_->training_iteration() + save_freq_;
